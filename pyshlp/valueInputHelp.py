...
API = {
    ...
    "rfm_shlp_get": "/COE/RBP_FE_SHLP_GET",
}


class valueInput:
    def __init__(self, connection, user_parameters=None, api=API):
        self.__conn = connection
        if api is None:
            api = API
        self.rfm_domvalues_get = api["rfm_domvalues_get"]
        self.rfm_metadata_get = api["rfm_metadata_get"]
        self.rfm_shlp_get = api["rfm_shlp_get"]

        if user_parameters is None:
            # not supplied by the application, get user params here
            user = self.__conn.get_connection_attributes()["user"]
            self._SU3Params = self.__conn.call("BAPI_USER_GET_DETAIL", USERNAME=user)["PARAMETER"]
        else:
    ...
    def get_shelp_descriptor(self, shlpname, default=""):
        """
        ...
        if helptype in ("SH", "CT", "CH"):
            if shlpname not in self._Descriptors:
                self._Descriptors[shlpname] = self.__conn.call(
                    self.rfm_metadata_get,
                    IV_SHLPNAME=helpname,
                    IV_SHLPTYPE=helptype,
                    IV_DEFAULT_SHLPNAME=default,
                )
                # print helpname
            ...
            sort_order = (
                []
            )  # todo keep the order in python JSON serializer and remove this parameter
            for shlp in self._Descriptors[shlpname]["ET_SHLP"]:
                elem_shlpname = "%s %s" % (shlp["SHLPTYPE"], shlp["SHLPNAME"])
                sort_order.append(elem_shlpname)
                elementary_helps[elem_shlpname] = self._ElementaryHelp[elem_shlpname]

            return {"elementary_helps": elementary_helps, "sort_order": sort_order}
        else:
            raise Exception("search help type %s not supported, only SH, CH and CT" % helptype)

    def get_field_values(self, fv_helpname):
        if fv_helpname not in self._FieldDomainValues:
            # _todo: error handling, like wrong domain name ...
            self._FieldDomainValues[fv_helpname] = self.__conn.call(
                self.rfm_domvalues_get, IV_DOMNAME=fv_helpname
            )
        return self._FieldDomainValues[fv_helpname]
    ...
    def search(self, shlpname, select_options, maxrows=0, compact=False):
        ...
        headers = []
        if "SHLPISPOS" in self._ElementaryHelp[shlpname]["FIELDDESCR"][0]:
            ...
            headers = sorted(headers, key=lambda k: k["position"])
        else:
            for f in self._ElementaryHelp[shlpname]["FIELDDESCR"]:
                headers.append(
                    {
        ...
        if len(result) > 0:
            if compact:
                ...
                result = sorted(result, key=lambda k: k[headers[0]["field"]])
        # maxrows exceeded
        ...
        return {
            ...
            "shlpoutput": shlpoutput,
            "maxrows_exceeded": exceeded,
            "headers": headers,
            "desc": r["ET_VALUE_DESC"],
        }
